...
# ------------------ Helpers ------------------
def ensure_unique(df: pd.DataFrame, boezem: Optional[bool] = False) -> pd.DataFrame:
    ...
    if boezem:
        df["SORT_CODE"] = (
            df["CODE"].astype(str)
            + df["NAAM"].astype(str).str.split("_").str[-1].str.split("-").str[:-1].str.join("-")
        )
    else:
...
gdf = gpd.read_file('./02_Intermediate_shapefiles/Hecto_Dijkring_Join.shp').to_crs(epsg=4326)
...
gdf2 = gpd.read_file('./02_Intermediate_shapefiles/WSHD_Boezem_Join.shp').to_crs(epsg=4326)
...
# Natuurlijke sortering op unique_, en bij dubbele unique_ de 'primair' prefereren
...
# ------------------ Data: lijnen ------------------
line_prim = gpd.read_file('./02_Intermediate_shapefiles/dwp_2025_prim_kering_hecto.shp').to_crs(epsg=4326)
line_prim = ensure_unique(line_prim)

line_boez_links = gpd.read_file('./02_Intermediate_shapefiles/dwp_2025_boezem_links_hecto.shp').to_crs(epsg=4326)
line_boez_links = ensure_unique(line_boez_links)

line_boez_rechts = gpd.read_file('./02_Intermediate_shapefiles/dwp_2025_boezem_rechts_hecto.shp').to_crs(epsg=4326)
line_boez_rechts = ensure_unique(line_boez_rechts)

# ------------------ Figure maker ------------------
def make_map_with_highlight(idx: int, zoom: int, show_basis: bool, show_boezem: bool, show_prim: bool, show_boez_lines: bool) -> go.Figure:
    """Kaart met één puntenlaag (gekleurd op 'source') en alleen de lijnen bij de selectie.
    ...
    fig.update_layout(
        map=dict(style="open-street-map", center=dict(lat=row.lat, lon=row.lon), zoom=zoom),
    ...
    if show_boez_lines:
        if boezem_side == 'links':
            sel_boez = line_boez_links[line_boez_links['unique_'] == row['unique_']]
        else:
            sel_boez = line_boez_rechts[line_boez_rechts['unique_'] == row['unique_']]
    ...
    if not drew_any and (show_prim or show_boez_lines):
        fig.update_layout(title_text=f"Geen lijnen gevonden voor unique_ = {row['unique_']}")
    ...
    [
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H1('Hectometrering', style={'textAlign': 'center'}),
                            ...
                            [
                                dbc.Col(
                                    dcc.Checklist(
                                        id='layer-toggle',
                                        options=[
                                            {'label': 'Punten (primair)',  'value': 'primair'},
                                            {'label': 'Punten (boezem)', 'value': 'boezem'},
                                            {'label': 'Lijn (primair)',  'value': 'line_prim'},
                                            {'label': 'Lijn (boezem)',   'value': 'line_boez'},
                                        ],
                                        value=['primair', 'boezem', 'line_prim', 'line_boez'],
                                        inline=True,
                ...
                dbc.Col(
                    [
                        dbc.Button('Copy', id='copy-button', color="primary", className='me-1'),
                        dash_table.DataTable(
                            id='hover-table',
                            page_size=500,
                            virtualization=True,
                            style_table={'height': f'{MAP_HEIGHT}px', 'overflowY': 'auto'},
                        ),
                    ],
                    width=3
                ),
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H2('AHN Dwarsprofiel', style={'textAlign':'center'}),
                        dbc.ButtonGroup(
                            [
                                dbc.Button("◀ Vorige", id="prev-btn", color="secondary", className="me-2"),
                                dbc.Button("Volgende ▶", id="next-btn", color="secondary"),
                                dbc.Button("Boezem Links", id="switch-btn", color="primary"),
                            ],
                            className="mb-2",
                        ),
                        dcc.Graph(id='profile-fig', style={'height': f'{PROFILE_HEIGHT}px'}),
                        dcc.RangeSlider(-200, 150, 10, value=[-200, 150], id="profile-rangeslider", 
...
def navigate_points(prev_clicks, next_clicks, clickData, layers, idx):
    """Werk actieve index bij o.b.v. knoppen/klik en herteken kaart o.b.v. toggles."""
    # visibility flags
    ...
    fig = make_map_with_highlight(idx, DEFAULT_ZOOM, show_basis, show_boezem, show_prim, show_boez_lines)
    return idx, fig
...
def update_detail(idx, rangevalue: Tuple[int, int]):
    ...
    empty_fig.update_layout(margin=dict(r=0, t=40, l=0, b=0), height=PROFILE_HEIGHT, autosize=False)
    ...
    def read_bathy_csv(path):
        ...
        if 'xRD' not in df.columns:
            raise ValueError(f"Kolom 'xRD' niet gevonden in {os.path.basename(path)}")

        # Alle kolommen strikt vóór xRD (xRD uitgesloten)
        idx_xrd = df.columns.get_loc('xRD')
        hoogtekolommen = list(df.columns[1:idx_xrd])  # strikt vóór xRD en niet de index

        # Verwachtte bronkolommen
        if 'meters' not in df.columns:
            raise ValueError(f"Kolom 'meters' niet gevonden in {os.path.basename(path)}")
        if 'ahn' not in df.columns:
            raise ValueError(f"Kolom 'ahn' niet gevonden in {os.path.basename(path)}")
        ...
        # Fallback: eerste geldige waarde uit kolommen vóór xRD, exclusief meters/ahn
        fallback_kolommen = [c for c in hoogtekolommen if c not in ('meters', 'ahn')]
    ...
    for offset in [-1, 0, 1]:
        ...
        matches = sorted(glob.glob(os.path.join('./03_Bathymetry', f"dp_cpt_{file_name_comp}_*.csv")))

        if not matches:
            # Als er helemaal niets is voor de centrale index, toon melding
            if offset == 0:
                msg_fig = px.scatter(title=f"Geen CSV gevonden voor: {file_name_comp}")
                msg_fig.update_layout(margin=dict(r=0, t=40, l=0, b=0), height=PROFILE_HEIGHT, autosize=False)
                return table_cols, table_data, msg_fig
            # voor buurprofielen gewoon overslaan
            continue

        # Kies CSV op basis van boezemzijde (gecontroleerd, niet blind op index 1)
        pick = 0
        if boezem_side == 'rechts' and len(matches) > 1:
            pick = 1  # of kies op naam/regex als je L/R expliciet in bestandsnaam hebt
        ...
        try:
            ...
            if offset == 0:
                bad = px.scatter(title=f"Probleem met CSV: {os.path.basename(csv_path)} — {e}")
                bad.update_layout(margin=dict(r=0, t=40, l=0, b=0), height=PROFILE_HEIGHT, autosize=False)
        ...
        if offset == 0:
            if "Primair" in csv_path:
                table_data = df[['x', 'z']].loc[rangevalue[0]*2+400 : rangevalue[1]*2+401].to_dict('records')
            else:
                table_data = df[['x', 'z']].loc[rangevalue[0]*2+300 : rangevalue[1]*2+300].to_dict('records')
